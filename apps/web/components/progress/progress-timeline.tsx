  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white px-3 py-2 shadow-lg text-sm">
      ...
      <p className="text-zinc-600">
        Score: <span className="font-bold text-emerald-600">{d.overall_skin_score?.toFixed(1)}</span>
      </p>
      {d.delta_from_baseline != null && d.delta_from_baseline !== 0 && (
        <p className={d.delta_from_baseline > 0 ? "text-emerald-600" : "text-rose-500"}>
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-6">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-3xl font-bold text-emerald-600">{latestScore?.toFixed(0) ?? "—"}</span>
        </div>
        {totalImprovement != null && totalImprovement !== 0 && (
          <div className="flex flex-col">
            <span className="text-xs text-zinc-500 uppercase tracking-wide">Improvement</span>
            <span
              className={`text-3xl font-bold ${totalImprovement > 0 ? "text-emerald-600" : "text-rose-500"}`}
            >
              {totalImprovement > 0 ? "+" : ""}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
            ...
            <ReferenceLine
              ...
              stroke="#d1d5db"
              strokeDasharray="4 4"
              label={{ value: "Healthy baseline (80)", position: "insideTopRight", fontSize: 10, fill: "#9ca3af" }}
            />
            <Area
              type="monotone"
              dataKey="score"
